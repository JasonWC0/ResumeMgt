 ...
 *
 * File: v1-role-authorization-read.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-04-19 11:38:27 am
...
describe('讀取公司角色權限資料', () => {
  ...
  describe('成功', () => {
    ...
    test('[200] 讀取公司角色權限資料-成功', async (done) => {
      const res = await agent
        .get(`${_ENDPOINT}/${id}`)
        .set({
          Authorization: `Bearer ${loginData.token}`,
      ...
      expect(res.body.result.role).toBe(data.role);
      expect(res.body.result.name).toBe(data.name);
      expect(res.body.result.manageAuthLevel).toBe(data.manageAuthLevel);
      expect(res.body.result.isDefault).toBe(data.isDefault);
