 ...
 *
 * File: v1-medicine-update.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-08-11 11:37:35 am
...
describe('更新藥品資料', () => {
  ...
  describe('成功', () => {
    test('[200] 更新藥品資料-成功', async (done) => {
      ...
      expect(medRes.indications).toBe(data.indications);
      expect(medRes.sideEffects).toBe(data.sideEffects);
      expect(medRes.healthEducation).toBe(data.healthEducation);
      expect(medRes.storageConditions).toBe(data.storageConditions);
      expect(medRes.usageInfo).toBe(data.usageInfo);
      expect(medRes.remark).toBe(data.remark);
