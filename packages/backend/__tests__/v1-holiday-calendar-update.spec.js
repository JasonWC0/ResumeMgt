 ...
 *
 * File: v1-holiday-calendar-update.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-07-25 02:42:24 pm
...
describe('更新假日行事曆', () => {
  ...
  beforeAll(async (done) => {
    ...
    await calendarCol.insert([_data, _data2]);
    const res = await calendarCol.findOne({ valid: _data.valid, companyId: _data.companyId, date: _data.date, type: _data.type });
    id = res._id.toString();
    done();
  });
  ...
  describe('需求欄位檢查', () => {
    test('[400:11125] 日曆日期空白', async (done) => {
      ...
      _data.date = '';
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}/${id}`, headers, _data);
  ...
  describe('成功', () => {
    test('[200] 更新假日行事曆-成功', async (done) => {
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}/${id}`, headers, data);

      const calendarCol = NativeDBClient.getCollection(modelCodes.CALENDAR);
      const calendarRes = await calendarCol.findOne({ companyId: new ObjectId(testCompanyData[0]._id), date: new Date(data.date), valid: true });
