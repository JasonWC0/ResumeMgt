 ...
 *
 * File: v1-company-institution-setting-obj-read.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-05-19 06:27:32 pm
...
describe('取得公司進階服務設定特定欄位', () => {
  ...
  describe('需求欄位檢查', () => {
    test('[404:90008] 公司Id不存在', async (done) => {
      const res = await agent
        .get(`${_ENDPOINT}/0000x0000x0000/institutionSetting/employeeCategory`)
        .set({
  ...
  describe('成功', () => {
    test('[200] 取得公司進階服務設定特定欄位-成功', async (done) => {
      const res = await agent
        .get(`${_ENDPOINT}/${testCompanyData.data[0]._id}/institutionSetting/employeeCategory`)
        .set({
          Authorization: `Bearer ${loginData.token}`,
          vn: '',
          sc: 'Erpv3',
