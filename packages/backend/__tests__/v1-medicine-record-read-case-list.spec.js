 ...
 *
 * File: v1-medicine-record-read-case-list.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-09-02 03:18:14 pm
...
describe('取得用藥提醒紀錄個案列表', () => {
  ...
  describe('成功', () => {
    test('[200] 取得用藥提醒紀錄個案列表-成功', async (done) => {
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}`, headers, {}, query);

      expect(res.status).toBe(200);
      expect(res.body.result).toHaveLength(1);
      expect(res.body.result[0].caseId).toBe(caseId);
      done();
    });
  });
});
