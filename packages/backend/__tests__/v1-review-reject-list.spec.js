 ...
 *
 * File: v1-review-reject-list.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-05-17 01:58:50 pm
...
describe('批次審核駁回', () => {
  const _ENDPOINT = '/main/app/api/v1/reviewForms/s/reject';
  let agent;
  let loginData;
  let sid1;
  let sid2;
  ...
  describe('需求欄位檢查', () => {
    test("[400:10520] 批次審核駁回資料列表空白", async (done) => {
    ...
    test("[400:10514] 批次審核駁回-其中表單狀態Id空白", async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}`)
        .send({ data: [{ id: '', vn: 0 },] })
        .set({
    ...
    test("[400:10018] 批次審核駁回-其中表單狀態vn空白", async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}`)
  ...
  describe('成功', () => {
    test("[200] 批次審核駁回-成功", async (done) => {
      ...
      expect(res.body.result.fail.length).toBe(2);
      expect(statusRes1.status).toBe(3);
      expect(statusRes1.lastReviewerName).toBe('管理員');
      expect(statusRes1.layerAt).toBe(null);
      expect(statusRes1.nowReviewRole).toEqual(null);
      expect(statusRes1.__vn).toBe(1);
      expect(statusRes1.__sc).toBe('Erpv3');
      expect(statusRes1.__cc).toBe(res.body.traceId);
      expect(statusRes2.status).toBe(3);
      expect(statusRes2.lastReviewerName).toBe('管理員');
      expect(statusRes2.layerAt).toBe(null);
      expect(statusRes2.nowReviewRole).toEqual(null);
