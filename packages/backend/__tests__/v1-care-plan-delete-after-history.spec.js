 ...
 *
 * File: v1-care-plan-delete-after-history.spec.js
...
describe('刪除後筆歷史照顧計畫', () => {
  ...
  beforeAll(async (done) => {
    ...
    carePlanData3 = {
      ...
      caseId: caseId,
      planType: 0,
      consultingDate: new Date('2020/07/20'),
      acceptConsultingDate: new Date('2020/07/20'),
      visitingDate: new Date('2020/07/20'),
      firstServiceDate: new Date('2020/07/20'),
      reliefType: 1,
      cmsLevel: 3,
      disabilityCategoryType: 1,
      newDisabilityCategories: [1, 2],
      ...
      pricingType: 0,
      foreignCareSPAllowance: false,
      planStartDate: new Date('2020/07/16'),
      planEndDate: null,
      introduction: '簡述',
      subjectChangeSummary: '主旨',
      changeReason: '異動原因',
    ...
    const optCarePlanData2 = {
      ...
      pricingType: 0,
      foreignCareSPAllowance: false,
      planStartDate: new Date('2020/05/01'),
      introduction: '簡述',
      subjectChangeSummary: '主旨',
      changeReason: '異動原因',
      declaredServiceCategory: [1, 2],
    ...
    const optCarePlanData3 = {
      ...
      pricingType: 0,
      foreignCareSPAllowance: false,
      planStartDate: new Date('2020/06/01'),
      introduction: '簡述',
      subjectChangeSummary: '主旨',
      changeReason: '異動原因',
      declaredServiceCategory: [1, 2],
    ...
    carePlanCol.insertMany([carePlanData1, carePlanData2, carePlanData3, optCarePlanData1, optCarePlanData2, optCarePlanData3]);
    done();
  });
  ...
  describe('成功', () => {
    test('[200] 刪除後筆歷史照顧計畫-成功', async (done) => {
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}/${carePlanData2._id.toString()}`, headers);

      const carePlanCol = NativeDBClient.getCollection(modelCodes.CAREPLAN);
      const careRes1 = await carePlanCol.findOne({ _id: new ObjectId(carePlanData2._id) });
      ...
      const newOptCareRes = await carePlanCol.findOne({ planType: 1, planStartDate: new Date('2020/07/01'), valid: true });

      expect(res.status).toBe(200);
      expect(careRes1.valid).toBe(false);
      expect(careRes2.planEndDate).toEqual(carePlanData2.planEndDate);
      expect(optCareRes.length).toBe(4);
      expect(newOptCareRes.AA09Declared).toEqual(carePlanData3.AA09Declared);
      for (const item of newOptCareRes.serviceItems) {
        if (item.itemId === carePlanData3.serviceItems[0].itemId) {
          expect(item.amount).toBe(carePlanData3.serviceItems[0].amount);
        }
      }
      done();
    });
