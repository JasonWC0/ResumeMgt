 ...
 *
 * File: v1-review-approve.spec.js
 * Project: @erpv3/backend
 * File Created: 2022-05-13 03:27:12 pm
...
describe('審核通過', () => {
  ...
  describe('資料規則驗證', () => {
    test('[404:90008] 此sid不存在', async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/ooooxxxxooooxxxx/s/approve`)
        .send({ comment: '填寫不完整' })
        .set({
    ...
    test('[400:20509] 無權限變更審核狀態', async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/${sid3}/s/approve`)
        .send({ comment: '填寫不完整' })
        .set({
          Authorization: `Bearer ${loginData.token}`,
          vn: 0,
    ...
    test('[400:20508] 審核狀態鎖定', async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/${sid4}/s/approve`)
        .send({ comment: '填寫不完整' })
        .set({
          Authorization: `Bearer ${loginData.token}`,
          vn: 0,
    ...
    test('[400:90010] 資料版號有誤(低於現有版本)', async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/${sid1}/s/approve`)
        .send({ comment: 'ok' })
        .set({
          Authorization: `Bearer ${loginData.token}`,
          vn: -1,
  ...
  describe('成功', () => {
    test("[200] 審核通過-成功 (審核流程結束)", async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/${sid1}/s/approve`)
        .send({ comment: 'ok' })
        .set({
      ...
      expect(statusRes.status).toBe(4);
      expect(statusRes.comment).toBe('ok');
    ...
    test("[200] 審核通過-成功 (審核流程尚未結束)", async (done) => {
      const res = await agent
        .patch(`${_ENDPOINT}/${sid2}/s/approve`)
        .send({ comment: 'ok' })
        .set({
          Authorization: `Bearer ${loginData.token}`,
          vn: 0,
      ...
      const statusCol = NativeDBClient.getCollection(modelCodes.FORMREVIEWSTATUS);
      const statusRes = await statusCol.findOne({ _id: ObjectId(sid2) });
      const historyCol = NativeDBClient.getCollection(modelCodes.FORMREVIEWHISTORY);
      const histRes = await historyCol.find({ fid: ObjectId(statusRes.fid) }).toArray();
      ...
      expect(statusRes.comment).toBe('ok');
      expect(statusRes.lastReviewerName).toBe('管理員');
      expect(statusRes.layerAt).toBe(1);
      expect(statusRes.nowReviewRole).toEqual([10]);
      expect(histRes.length).toBe(2);
      for (const hist of histRes) {
        if (hist.status === 4) {
          expect(hist.comment).toBe('ok');
