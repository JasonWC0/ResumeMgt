...
describe('編輯個案合約', () => {
  ...
  describe('需求欄位檢查', () => {
    test('[400:11503] 合約服務類型空白', async (done) => {
      ...
      _data.service = '';
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}/${testCaseServiceContractData[0]._id}`, headers, _data);
  ...
  describe('成功', () => {
    test('[200] 讀取個案合約列表-成功', async (done) => {
      const res = await Base.callAPI(agent, METHOD, `${_ENDPOINT}/${testCaseServiceContractData[0]._id}`, headers, data);
