/**
 * FeaturePath: Common-Utility--格式檢查
 ...
 * File Created: 2022-02-08 10:08:20 am
...
const _namesEnum = {
  ...
  NON_EMPTY_OBJECT: 'isNonEmptyObject',
  IS_EMAIL: 'isEmail',
  IS_BOOL: 'isBoolean',
  IS_DATE: 'isDate',
  IS_BEFORE: 'isBefore',
  IS_NUM: 'isNumber',
  IS_ISODATE: 'isISODate',
};
...
* @private
* @function
* @description Validate if input array is empty
* @param {Object[]} val
* @returns {boolean} boolean
*/
...
* @private
* @function
* @description Validate if object is empty
* @param {Object[]} val
* @returns {boolean} boolean
*/
function _nonEmptyObject(val) {
...
function _isBoolean(val) {
  ...
  if (typeof val === 'string') {
    return _validator.isBoolean(val);
  }
  return false;
}
...
const _strategies = new Map()
  ...
  .set(_namesEnum.NON_EMPTY_OBJECT, _nonEmptyObject)
  .set(_namesEnum.IS_EMAIL, _validator.isEmail)
  .set(_namesEnum.IS_BOOL, _isBoolean)
  .set(_namesEnum.IS_DATE, _validator.isDate)
  .set(_namesEnum.IS_NUM, _isNumber)
  .set(_namesEnum.IS_BEFORE, _validator.isBefore)
  .set(_namesEnum.IS_ISODATE, _validator.isISO8601);
 ...
 */
  ...
  */
  ...
  /**
  * @description Validate if input object is not empty object
  * @static
  * @param {string} val - Input value
  * @param {string} [message=''] - Message while checking fail
  * @returns {boolean} boolean
  * @throws {CustomError} Throws error when message is not empty
  */
  static nonEmptyObject(val, message = '') {
    const result = _nonEmptyObject(val);
    if (!result && _nonEmptyString(message)) {
      throw new CustomError(message);
    }
    return result;
  ...
  */
  ...
  static isBoolean(val, message) {
    const result = _isBoolean(val);
    if (!result && _nonEmptyString(message)) {
      throw new CustomError(message);
    }
    return result;
