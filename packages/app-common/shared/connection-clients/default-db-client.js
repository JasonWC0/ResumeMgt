/**
 * FeaturePath: 通用主系統-通用子系統-連線模組-MongooseMongoDB
 ...
 *
 * File: default-db-client.js
 * Project: @erpv3/app-common
 * File Created: 2022-02-08 10:14:12 am
 ...
 */
  static async syncIndexes() {
    if (_REJ_CLN_ENV.includes(customArgvs.env)) {
      customLogger.info(`Not allowed to sync index for env ${customArgvs.env}`);
      return;
    }
    const tasks = [];
    for await (const [k, v] of _docs.entries()) {
      customLogger.info(`Default Db client - sync ${k} indexes`);
      await v.syncIndexes();
      // tasks.push(v.syncIndexes());
    }
    // await Promise.all(tasks);
  }

  static takeConnection() {
    return _conn;
  }
}
